<template>
  <div class="sub_container">
    <h3 class="sub-title1 my-3">팝업1</h3>
    <div class="my-3">
      <v-btn
        class="mr-1"
        color="red lighten-2"
    ...
    <div class="my-3">
      <v-btn
        ...
        dark
        @click="$dialog.alert('글로벌 Alert 팝업 오픈')"
      >
        글로벌 Alert
      </v-btn>
      <v-btn
        ...
        dark
        @click="$dialog.confirm('글로벌 Confirm 팝업 오픈')"
      >
        글로벌 Confirm
      </v-btn>
    </div>
...
export default {
  ...
  methods: {
    windowPopupOpen () {
      const popupName = 'TemplateWindowPopup1'
      const routeData = this.$router.resolve({ name: popupName })
      const windowRef = window.open(routeData.href, popupName, 'width=600,height=300,left=20,top=20')
